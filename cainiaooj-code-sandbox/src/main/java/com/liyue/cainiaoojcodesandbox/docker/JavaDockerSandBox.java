...
@Slf4j
@Component
public class JavaDockerSandBox extends CodeSandboxTemplate {
    ...
    static {
        ...
        WORD_TREE.addWords(CodeBlackList.JAVA_SENSITIVE_WORD_LIST);
    }
    public JavaDockerSandBox(DockerContainerPool dockerContainerPool, DockerClient dockerClient){
    ...
    public ExecuteCodeResponse executeCode(ExecuteCodeRequest executeCodeRequest) {
        ...
        File userCodeFile = saveCodeToFile(code,GLOBAL_CODE_PATH_NAME,GLOBAL_JAVA_CLASS_NAME);
        try {

            ExecuteMessage compileFileExecuteMessage = compileFile(CommandManger.getComplieCommands(language,userCodeFile));
        } catch (Exception e) {
            log.error("编译出错了");
