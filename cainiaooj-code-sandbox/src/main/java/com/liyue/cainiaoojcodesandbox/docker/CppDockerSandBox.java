...
public class CppDockerSandBox extends CodeSandboxTemplate {
    ...
    static {
        ...
        WORD_TREE.addWords(CodeBlackList.C_SENSITIVE_WORD_LIST);
    }
    public CppDockerSandBox(DockerContainerPool dockerContainerPool, DockerClient dockerClient){
    ...
    public ExecuteCodeResponse executeCode(ExecuteCodeRequest executeCodeRequest) {
        ...
        File userCodeFile = saveCodeToFile(code,GLOBAL_CODE_PATH_NAME,GLOBAL_CPP_NAME);
        try {
            String complieCommands = CommandManger.getComplieCommands(language, userCodeFile);
            log.info("编译的命令拼出来是:"+complieCommands);
            ExecuteMessage compileFileExecuteMessage = compileFile(complieCommands);
        } catch (Exception e) {
            log.error("编译出错了");
        ...
        try {
            //执行代码
            String[] commands = CommandManger.getCommands(language, userCodeFile);
            log.info("执行的命令拼出来是");
