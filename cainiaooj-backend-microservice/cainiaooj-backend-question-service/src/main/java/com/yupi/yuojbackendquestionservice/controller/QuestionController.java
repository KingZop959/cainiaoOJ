...
import com.yupi.yuojbackendquestionservice.sentinel.SentinelLimiterUtil;
...
public class QuestionController {
    ...
    @Resource
    private SentinelLimiterUtil sentinelLimiterUtil;
    ...
    public BaseResponse<Long> addQuestion(@RequestBody QuestionAddRequest questionAddRequest, HttpServletRequest request) {
        ...
        if (result){
            myMessageProducer.sendMessage("ai_exchange","ai_solution",GSON.toJson(question));
        }
    ...
    public BaseResponse<Boolean> deleteQuestion(@RequestBody DeleteRequest deleteRequest, HttpServletRequest request) {
        ...
        User user = userFeignClint.getLoginUser(request);
        long id = deleteRequest.getId();
        // 判断是否存在
        Question oldQuestion = questionService.getById(id);
        ThrowUtils.throwIf(oldQuestion == null, ErrorCode.NOT_FOUND_ERROR);
        // 仅本人或管理员可删除
        if (!oldQuestion.getUserId().equals(user.getId()) && !userFeignClint.isAdmin(user)) {
    ...
    public BaseResponse<Question> getQuestionById(long id, HttpServletRequest request) {
        ...
        User loginUser = userFeignClint.getLoginUser(request);
        // 不是本人或管理员，不能直接获取所有信息
        if (!question.getUserId().equals(loginUser.getId()) && !userFeignClint.isAdmin(loginUser)) {
            throw new BusinessException(ErrorCode.NO_AUTH_ERROR);
        }
    ...
    public BaseResponse<Boolean> editQuestion(@RequestBody QuestionEditRequest questionEditRequest, HttpServletRequest request) {
        ...
        ThrowUtils.throwIf(oldQuestion == null, ErrorCode.NOT_FOUND_ERROR);
        // 仅本人或管理员可编辑
        if (!oldQuestion.getUserId().equals(loginUser.getId()) && !userFeignClint.isAdmin(loginUser)) {
            throw new BusinessException(ErrorCode.NO_AUTH_ERROR);
        }
        boolean result = questionService.updateById(question);
        return ResultUtils.success(result);
    ...
    public BaseResponse<Long> doQuestionSubmit(@RequestBody QuestionSubmitAddRequest questionSubmitAddRequest,
        ...
        final User loginUser = userFeignClint.getLoginUser(request);
        long questionId = questionSubmitAddRequest.getQuestionId();
        // 组合为参数列表，让 Sentinel 能识别参数
        String codeHash = DigestUtil.md5Hex(questionSubmitAddRequest.getCode());
        String combinedKey = loginUser.getId() + "_" + questionId + "_" + codeHash;
        sentinelLimiterUtil.submitLimiter(combinedKey);
     ...
     * @return
     */
    @PostMapping("/question_submit/list/history")
    public BaseResponse<List<QuestionSubmit>> listQuestionSubmitById(@RequestBody QuestionSubmitQueryRequest questionSubmitQueryRequest,
                                                                         HttpServletRequest request) {
        final User loginUser = userFeignClint.getLoginUser(request);
        Long questionId = questionSubmitQueryRequest.getQuestionId();
        List<QuestionSubmit> questionSubmitsById = questionSubmitService.getQuestionSubmitsById(loginUser, questionId);
        return ResultUtils.success(questionSubmitsById);
    }
